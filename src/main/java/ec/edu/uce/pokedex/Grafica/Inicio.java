...
public class Inicio extends JFrame {
    ...
    public Inicio() {
        ...
        typeComboBox = new JComboBox<>(new String[] {"All Types", "Fire", "Water", "Grass", "Electric"});  // Populate dynamically
        regionComboBox = new JComboBox<>(new String[] {"All Regions", "Kanto", "Johto", "Hoenn", "Sinnoh"});  // Populate dynamically
        abilityComboBox = new JComboBox<>(new String[] {"All Abilities", "Overgrow", "Blaze", "Torrent", "Levitate"}); // Populate dynamically
        moveComboBox = new JComboBox<>(new String[] {"All Moves", "Tackle", "Flamethrower", "Thunderbolt"}); // Populate dynamically
        habitatComboBox = new JComboBox<>(new String[] {"All Habitats", "Forest", "Cave", "Water", "Desert"}); // Populate dynamically
    ...
    private void searchPokemon(JLabel pageInfo) {
        ...
        String ability = (String) abilityComboBox.getSelectedItem();
        String move = (String) moveComboBox.getSelectedItem();
        String habitat = (String) habitatComboBox.getSelectedItem();

        if (query.isEmpty() && type.equals("All Types") && region.equals("All Regions") && ability.equals("All Abilities") && move.equals("All Moves") && habitat.equals("All Habitats")) {
            JOptionPane.showMessageDialog(this, "Please select at least one filter.");
            return;
        }

        // Llamar a los repositorios según el filtro seleccionado
        List<Pokemon> filteredPokemons = fetchFilteredPokemons(query, type, region, ability, move, habitat);

        // Muestra los Pokémon filtrados
        displayFilteredPokemons(filteredPokemons);
    }

    private List<Pokemon> fetchFilteredPokemons(String name, String type, String region, String ability, String move, String habitat) {
        // Aquí iría la lógica de la consulta para cada filtro.
        // Consulta los repositorios de tu base de datos y filtra los Pokémon de acuerdo con los parámetros.
        return null;  // Retorna una lista de Pokémon filtrados
    }
